...
def getNearbyScans(reader, scanNumber, options):
    scans = []
    nScans = options["number_nearby_scans"]
    MAX_SCAN_NUM = len(reader.index)
...
def eachJob(reader, scan, scanNum, options)
    # Get nearby scans
    nearbyScans = getNearbyScans(reader, scanNum, options)

    # Run Monocle algorithm
    precMz, precZ = monocle(scan, nearbyScans, options)
